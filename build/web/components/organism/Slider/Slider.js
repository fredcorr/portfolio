...
const Slider = (props) => {
  ...
  const calculateAmountRange = (endPoint) => {
    let amoutInPixel =
      calculateDistWindow(imagesRef[endPoint]).left -
      (window.innerWidth - imagesRef[endPoint].clientWidth) / 2
  ...
  const slideAimation = (direction, startPoint, endPoint, onStart) => {
    ...
    TweenMax.to(sliderInner.current, range / divider, {
      x: direction + amount + '%',
      ease: Power4.InOut,
      onStart: () => onStart,
    })
