...
const Project = ({cover, content, ...props}: any) => {
  ...
  return (
    <motion.div className={ styles.CaseStudy } exit={{ opacity: 0 }} animate={{ opacity: 1 }} initial={{ opacity: 0 }} >
      <Seo metas={ props.seo_details } title={ props.title } og_image={ cover.url } path={ props.slug.current }/>
        ...
        <ScrollFade>
          {
            (anim: any) => (
              <div ref={ anim.ref } className={ anim.style }>
                <Image src={content.hero_img.url} layout="responsive" width={width} height={height} placeholder="blur" blurDataURL={content.hero_img.metadata.lqip}/>
              </div>
            )
          }
        </ScrollFade>
      ...
      <section className={ styles.mainContent }>
        {
          content ? content.modules.map( ( module: any, i: number ) => {
            switch (module._type) {
              case 'text_module':
              ...
              case 'img':
                return (
                  <IntersectionObserver threshold={ 0 } key={ module._key } >{
                    (observer: any) => { 
                      const { width, height } = module.metadata.dimensions
                      return (
                        <motion.div 
                          ...
                          exit="hidden"
                          >
                          <Image src={module.asset.url} layout="responsive" width={width} height={height} placeholder="blur" blurDataURL={module.metadata.lqip}/>
                        </motion.div>
                    )}}
